...
data_final_level <- data_reduced %>%
  select(y, industrial_inputs, metals, energy,
         shipping, fx,industrial_prod,
         construction_licences_area, finished_constructions,google_trends,
         unemployment, interest_rate)
...
summary_stats <- sapply(data_final_level, custom_summary)

# Transpose the summary statistics table
summary_transposed <- t(summary_stats)

# Convert to data frame for better readability
summary_transposed_df <- as.data.frame(summary_transposed)

# Print the transposed summary
print(summary_transposed_df)
...
predictions <- predict(model, newdata = data_final_levels)
actuals <- data_final_levels$y  # Replace 'Y' with the dependent variable name
