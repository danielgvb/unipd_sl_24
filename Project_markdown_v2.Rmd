...
```
The final reduced model continue to show a high R-squared, while the QQ-Plot shows similar points outside the reference line. At this point even the fact that now all the variables are statistically significant, the model appears to has more or less the same explanatory power due to the residuals behaviour.
...
We apply a similar procedure for the ridge regression to see if we get a different model with respecto to lasso, as it uses a different regularization term. We rpesent the results.  
