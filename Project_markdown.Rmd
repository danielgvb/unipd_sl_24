...
data <- read_excel("C:/Users/CAMILO/Documents/clases/statistical_project/data.xlsx",sheet = "dataframe_col")
View(data)
...
After have a initial version of the data we realized the existence of NA values in some variables due to the time span of the data collection. For this reason we implement the following process to clean the data and the construction of some values for the finished constructions variable.
...
```
(Not implemented as the result is no good)
As we are interested in use the House Price Index as regressor related with the effect of the other macroeconomic variables, for that reason we implement a lag of one period over this variable
...
Then we proceed to remove the left rows with NAN values, drop the columns of date related with the time period of the indicators and the column MY also referred to the date in form of MONTH()YEAR(). Finally we present a summary of the final dataset. This taking account thet we leave the data starting from January 2005 untill August 2023.
...
As part of the explorator analysis we study the percentual variation of the variables. This process helps to identify trends and patterns in a clearly way. 
...
# Log-transformation
We apply a log transformation with the intention of normalize the data as some variables are index that change over time and others area real values in a wide range. Log helps to reduce the variability of the data, compressing the scale and helps to linearize those relationships between the variables helping to the interpret ability of the results. We apply log transformation to all variables except those that are already percentages: the unemployment rate and the interest rate.  
```{r}
...
Continuing with the exploratory analysis we plot the variable of Home Price Index to see its behavior through time. It is possible to see a clear tendency starting from the year 2005 until the year 2021, then a little decay was recorded. 
```{r}
# reset margins
par(mfrow = c(1, 1))
...
```
Another useful information can be find in the box plots and histograms fo the percentual variations,in these plots we see that the central value of the percentual variations is positive, indicating and overall increase. the outliers were expected in the senseo of those negatives variations due to the decrease on on the last years of the House Price Index. Also,  it is possible to see this on the histogram a right skewed behavior. 
```{r}
...
```
For an easier interpretation we plot the boxplot for the percentual variation of each variable in a same graph. This plot shows that the variables with a high variability in percentual variation are the shipping index price and the energy index price, in other hand the ones that show a more stable variability are the unemployment rate and interest rate. 
```{r}
# Reset layout
par(mfrow = c(1, 1))
...
We present the correlation and covariance of the used variables in aims to understand its relations. 
...
# Correlation matrix
cor_matrix <- cor(select(df_not_log,-date,-category), use = "complete.obs")
print(cor_matrix)
...
As part of the analysis we present a multicollinearity test in top of the results related with the correlation of the variables. We first calculate the VIF for each predictor variable, and remove the highly collinear ones
...
We start with a simple regression model for the data, remember that this process will be executed with the variables after the log normalization. In this first iteration considering all the variables 
```{r}
# OLS 1 Log-Log-----------------
...
```
After the first iteration we consider the process of eliminating variables based on its significance to the model
```{r}
...
```
This is the process made with the laggedy  value with  the intention of reduce the Homoskedasticity but it does not work so good 
```{r}
# Homoskedasticity
...
This process consider the model with robust std erros, we could think in leave it or not. 
...
Implement the forward selection based on the AIC obtained by the scores
...
Implement the backward selection based on the AIC obtained by the scores
