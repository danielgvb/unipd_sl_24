...
cov_matrix <- cov(select(df_not_log,-date))
cov_matrix

correl_matrix <- cor(select(df_not_log,-date))
correl_matrix
# Heatmap of correl
...
```
We observe again the significance of the variables, and the next no significant variables that we decide to drop were Industrial Inputs Index, Finished Constructions, Construction Licence Area and Unemployment. Al of this after eliminating step by step each one following the previous procedure with the ANOVA F-test untill all the predictors were significative.  
...
This process consider the model with robust std erros, we could think in leave it or notn as we did not see it on class. 
